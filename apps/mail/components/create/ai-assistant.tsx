...
const animations = {
  container: {
    ...
    animate: (width: number) => ({ 
      ...
      opacity: 1,
      transition: { width: { type: "spring", stiffness: 250, damping: 35 }, opacity: { duration: 0.4 } }
    }),
    exit: { 
      width: 32, 
      opacity: 0,
...
const ContentPreview = ({ content, animations }: { content: string, animations: any }) => (
  ...
  >
    <div className="p-3">
      <div className="min-h-[150px] max-h-60 overflow-y-auto rounded-md p-1 text-sm">
...
export const AIAssistant = ({
  ...
  const [generatedContent, setGeneratedContent] = useState<{content: string; jsonContent: JSONContent} | null>(null);
  ...
  const handleAccept = () => {
    if (generatedContent && onContentGenerated) {
      onContentGenerated(generatedContent.jsonContent, suggestedSubject || undefined);
